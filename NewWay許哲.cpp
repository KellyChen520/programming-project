...
int main()
{
	...
	while ( ansK <= turnTimes ){
		...
		int xx[3]={0},yy[3]={0}, path = 0;//我把X1,X2,x3 & Y1 Y2 Y3變成矩陣,重新命名為xx & yy ,path代表我們最後要選哪條路 
		...
		if (minCost < lowCost){//表示三條路徑有一條成本 比不轉還要小 
			...
			ansDistance += sqrt( pow( turnPlace[2*ansK+3] - turnPlace[2*ansK+1] , 2) +pow( turnPlace[ansK+2] - turnPlace[2*ansK] , 2) );
			lowCost = minCost; ansK++;//把最低的風險值放進lowCost, 轉折次數加一 
			XX = xx[path]; YY = yy[path];// 存入轉折點座標，下次迴圈的時候繼續用 XX & YY 
		}
		else {//不轉的風險最小的話 
			ansDistance += sqrt ( pow(cutX, 2) + pow((cutX * (Yt - YY)/(Xt-XX)), 2));
