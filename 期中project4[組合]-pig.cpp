...
double straightLineRisk (int Xstart, int Xend, int Ystart, int Yend, int *X, int *Y, int *R, int *P, int m);
...
void findMinCost(float d,int m, int k,int turntimes,bool **inD, float lowcost, int circleMinX,int circleMaxX,int circleMinY,
...
int main()
{
	...
	*/
	...
	findMinCost(d, m, 1, turnTimes, inD, lowCost, circleMinX, circleMaxX, circleMinY, circleMaxY, turnPlace, 
	...
	{
		cout << ansK <<" ";
		for(int i = 0; i < 2*ansK-1; i += 2)
		{
			if(i == 2*ansK-2)
			cout << ansXY[2*i-2] << " " << ansXY[2*i-1];
			else
			cout << ansXY[2*i-2] << " " << ansXY[2*i-1] << " ";
		}
	...
	for(int i =0; i < 2*k + 4; i += 2)
	{
		if(allPoints[i] != -1)
		{
			...
			if(i == 0 )
			{
				...
				{
					Xend = allPoints[2*k + 2];
					Yend = allPoints[2*k + 3];
				}
	...
	for(int y = circleMinY; y <= circleMaxY; y++)
	{
		...
		{	
			...
			if(inD[y][x] == 1 )
			{
				...
				float cost= totalLineRisk(turnPlace, X, Y, R, P, m, k);//function(轉折點(x,y))+w(k-1);
				...
				findMinCost(d+1, m, 1, turnTimes, inD, lowCost, circleMinX, circleMaxX, circleMinY, circleMaxY, turnPlace, 
							ansK, ansXY, distance, X, Y, R, P);		
			}
		} 
	}
